...
void R_init_mdsOpt(DllInfo* info) {
...
extern "C"  void mmSph(double *X, double *r, double *str, double *ldist, double *udist, double *ldiss, double *udiss, double *eps, int *relax, int *dilation, int *N, int *P, int *maxit, int *report)
	...
	{//start void	Rprintf("initial stress= %f \n", str[0]);
		...
		double *bw1, *bw2;*/
			...
			dgesvd_(&jobu, &jobvt, &n, &n, A1, &n, singular_value, U_svd, &n, Vt_svd, &n,work, &lwork, &info,(size_t)n,(size_t)n);
	...
	{//start void	Rprintf("initial stress= %f \n", str[0]);
		...
		/*r-r-r-r-r-r-r-r-r-r-r-r-r-r-r-r-r-r-r-r-r-r-r-r-r-r*/
			for(s =0; s<p;s++){//s
				...
				/*Start: Renew X[,s]*/
				//XXXXXXXXXXXXXXXXXXXXXXXXXXXXX
				//dgesvd_(&jobu, &jobvt, &n, &n, A1, &n, singular_value, U_svd, &n, Vt_svd, &n, &info);
				dgesvd_(&jobu, &jobvt, &n, &n, A1, &n, singular_value, U_svd, &n, Vt_svd, &n, work, &lwork, &info,(size_t)n,(size_t)n );
				if(info != 0){
					//Rprintf("Error (SVD) code %d \n",info);
					break;
